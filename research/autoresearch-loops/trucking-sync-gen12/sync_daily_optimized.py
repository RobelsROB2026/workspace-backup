"""
Gen 10 optimizations (builds on Gen9 165k RPM):
  O1-O22: inherited from Gen9
  O23: Persistent HTTP connection pool for Phase 2 batch fetches.
       Gen9 used urllib.request which opens a new TCP+TLS connection per request.
       Gen10 uses http.client.HTTPSConnection with thread-local connection reuse,
       eliminating repeated TLS handshakes (~100-200ms each) across batch fetches.
       Each worker thread gets its own persistent connection to data.transportation.gov.
"""
...
if companies_to_insert and leads_batch:
    ...
    leads_vals = b",".join(cursor.mogrify(leads_tpl, row) for row in leads_batch)

    combined_sql = (
        b"""WITH comp_upsert AS (
...
SELECT ld.dot_number, ld.status, ld.notes, ld.lead_type
FROM (VALUES """ + leads_vals + b""") AS ld(dot_number, status, notes, lead_type)
WHERE ld.dot_number IN (SELECT dot_number FROM comp_upsert)
ON CONFLICT (dot_number) DO UPDATE SET
    status = 'New',
    ...
    print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via single CTE.")
