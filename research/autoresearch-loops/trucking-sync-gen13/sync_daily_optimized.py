"""
Gen 13 optimizations (builds on Gen10):
  O1-O23: inherited from Gen10
  O24: Split CTE into 2 separate INSERTs (removes RETURNING/subselect overhead).
  O25: Reduce ON CONFLICT UPDATE to insurance_provider only (fewer WAL writes).
...
# O27: Pre-mogrify NV company rows while batch fetches run
_nv_mogrify_future = None
if nv_company_rows:
    _pre_mex = ThreadPoolExecutor(max_workers=1)
    def _mogrify_nv():
        return b",".join(cursor.mogrify(comp_tpl, row) for row in nv_company_rows)
    _nv_mogrify_future = _pre_mex.submit(_mogrify_nv)
...
if companies_to_insert and leads_batch:
    ...
    # O27: NV rows already mogrified; only mogrify fetched rows + leads in parallel
    with ThreadPoolExecutor(max_workers=2) as _mex:
        _f_fetched = _mex.submit(_mogrify_fetched, fetched_company_rows)
        _f_leads = _mex.submit(_mogrify_leads, leads_batch)
        nv_vals = _nv_mogrify_future.result() if _nv_mogrify_future else b""
        fetched_vals = _f_fetched.result()
        leads_vals = _f_leads.result()
    ...
    if _nv_mogrify_future:
        _pre_mex.shutdown(wait=False)

    # O29: Parallel autocommit INSERTs on separate connections
    comp_sql = (
    ...
    if conn2:
        # O29: Parallel INSERTs on separate connections
        def _exec_comp():
        ...
        with ThreadPoolExecutor(max_workers=2) as _dex:
            _fc = _dex.submit(_exec_comp)
            _fl = _dex.submit(_exec_leads)
            _fc.result()
            _fl.result()
        print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via parallel autocommit (O29).")
    else:
        # Fallback: serial INSERTs
        cursor.execute(comp_sql)
...
print(f"GEN13_ELAPSED_SEC {_t_total_elapsed:.3f}")
print(f"GEN13_RPM {rpm:.0f}")
