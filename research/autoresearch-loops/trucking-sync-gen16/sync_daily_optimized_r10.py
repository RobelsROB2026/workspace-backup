...
# O47: Pre-check existing DOTs during Phase 2 (overlapped with Socrata fetches)
_existing_dots_result = [None]
def _precheck_existing_dots():
    """Query DB for existing DOTs — runs concurrently with batch fetches."""
    try:
        _all_dot_list = list(all_dots)
        cur2 = conn2.cursor()
        cur2.execute("SELECT dot_number FROM companies WHERE dot_number = ANY(%s)", (_all_dot_list,))
        _existing_dots_result[0] = {row[0] for row in cur2.fetchall()}
        cur2.close()
    except Exception as e:
        print(f"[WARN] O47 pre-check failed: {e}")
        _existing_dots_result[0] = None
...
with ThreadPoolExecutor(max_workers=MAX_BATCH_WORKERS + 1) as executor:
    # O47: Fire pre-check alongside batch fetches
    _f_precheck = executor.submit(_precheck_existing_dots)
...
_f_precheck.result()  # O47: ensure pre-check is done
_existing_dots = _existing_dots_result[0]
_fetch_elapsed = time.perf_counter() - _t_fetch
_precheck_str = f"existing={len(_existing_dots)}" if _existing_dots else "precheck=failed"
print(f"[PERF] Phase 2 (batch fetch+parse+precheck): {_fetch_elapsed*1000:.0f}ms | {len(batches)} batches | {_precheck_str}")

# O44+O47: Smart split — only INSERT new companies, UPDATE insurer for existing
...
if companies_to_insert and leads_batch:
    ...
    # O47: If pre-check succeeded, split into new INSERT + insurer UPDATE (skip existing DOTs entirely)
    if _existing_dots is not None:
        _new_comp_rows = [row for row in companies_to_insert if row[0] not in _existing_dots]
        _insurer_rows = [(row[0], row[11]) for row in companies_to_insert if row[0] in _existing_dots and row[11] is not None]
        print(f"[O47] Smart split: {len(_new_comp_rows)} new INSERTs + {len(_insurer_rows)} insurer UPDATEs (skipped {len(companies_to_insert)-len(_new_comp_rows)-len(_insurer_rows)} no-change)")
    else:
        # Fallback if pre-check failed: use DO NOTHING approach
        _new_comp_rows = companies_to_insert
        _insurer_rows = [(row[0], row[11]) for row in companies_to_insert if row[11] is not None]
        print(f"[O47] Precheck failed, using DO NOTHING fallback")

    if conn2:
        # O50: Build only new company VALUES (leads + insurer use unnest, no build needed)
        new_vals = None
        if _new_comp_rows:
            new_vals = _build_values_fast(_new_comp_rows)

        # O49: Unnest arrays for insurer UPDATE
        _insurer_dots = [r[0] for r in _insurer_rows] if _insurer_rows else []
        _insurer_vals_list = [r[1] for r in _insurer_rows] if _insurer_rows else []
        # O50: Unnest arrays for leads INSERT
        _leads_dots = [r[0] for r in leads_batch]
        _leads_status = [r[1] for r in leads_batch]
        _leads_notes = [r[2] for r in leads_batch]
        _leads_types = [r[3] for r in leads_batch]

        # O48: Fire new company INSERT on conn1, insurer UPDATE + leads INSERT in parallel
        # New companies are rare (~22/day), so conn1 finishes instantly
        # Then use both connections for insurer UPDATE + leads INSERT in parallel
        if new_vals:
            if _existing_dots is not None:
                cursor.execute(_COMP_PREFIX + new_vals)
            else:
                cursor.execute(_COMP_PREFIX + new_vals + b" ON CONFLICT (dot_number) DO NOTHING")
        # O48+O49: Fire insurer UPDATE (unnest) + leads INSERT in parallel on 2 connections
        def _exec_insurer():
            if _insurer_dots:
                cursor.execute(
                    "UPDATE companies SET insurance_provider = v.ins FROM unnest(%s::text[], %s::text[]) AS v(dot, ins) WHERE companies.dot_number = v.dot",
                    (_insurer_dots, _insurer_vals_list)
                )
        def _exec_leads():
            cur2 = conn2.cursor()
            # O50: Leads INSERT via unnest
            cur2.execute(
                """INSERT INTO leads (dot_number, status, agent_notes, lead_type)
                SELECT * FROM unnest(%s::text[], %s::text[], %s::text[], %s::text[])
                ON CONFLICT (dot_number) DO UPDATE SET status = 'New', lead_type = EXCLUDED.lead_type, updated_at = NOW()""",
                (_leads_dots, _leads_status, _leads_notes, _leads_types)
            )
            cur2.close()
        with ThreadPoolExecutor(max_workers=2) as _dex:
            _f1 = _dex.submit(_exec_insurer)
            _f2 = _dex.submit(_exec_leads)
            _f1.result()
            _f2.result()
        _t_build_elapsed = time.perf_counter() - _t_build
        print(f"[PERF] SQL build+exec (O47 smart-split): {_t_build_elapsed*1000:.0f}ms | new={len(_new_comp_rows)} upd={len(_insurer_rows)} leads={len(leads_batch)}")
        print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via smart-split (O47).")
    else:
