"""
Gen 15 optimizations (builds on Gen14):
  O1-O29: inherited from Gen13
  O30-O34: inherited from Gen14 (adapt() SQL, overlap, precompute leads, split parallel INSERT)
  O35+: See notes/autoresearch_plan.md for trial results
"""
...
if companies_to_insert and leads_batch:
    ...
    _COMP_SUFFIX = b" ON CONFLICT (dot_number) DO UPDATE SET insurance_provider = COALESCE(EXCLUDED.insurance_provider, companies.insurance_provider)"

    # O35+O40: Build company VALUES in parallel, fire them immediately, build leads during DB I/O
    if conn2 and _comp_rows_b:
        _build_exec = ThreadPoolExecutor(max_workers=2)
        _f_vals_a = _build_exec.submit(_build_values_fast, _comp_rows_a)
        _f_vals_b = _build_exec.submit(_build_values_fast, _comp_rows_b)

        # Wait for company VALUES, assemble SQL
        comp_vals_a = _f_vals_a.result()
        comp_vals_b = _f_vals_b.result()
        _build_exec.shutdown(wait=False)

        comp_sql1 = _COMP_PREFIX + comp_vals_a + _COMP_SUFFIX
        comp_sql2 = _COMP_PREFIX + comp_vals_b + _COMP_SUFFIX

        # O40: Fire company INSERTs + build leads VALUES concurrently
        _leads_sql_holder = [None]
        def _build_leads_sql():
        ...
        def _exec_half2():
            ...
            cur2.close()
        with ThreadPoolExecutor(max_workers=3) as _dex:
            _f1 = _dex.submit(_exec_half1)
            _f2 = _dex.submit(_exec_half2)
            _f_leads_build = _dex.submit(_build_leads_sql)
            _f1.result()
            _f2.result()
            _f_leads_build.result()
        # All companies committed + leads SQL ready — fire leads
        cursor.execute(_leads_sql_holder[0])
        _t_build_elapsed = time.perf_counter() - _t_build
        print(f"[PERF] SQL build+exec (O40 overlap): {_t_build_elapsed*1000:.0f}ms | A={len(_comp_rows_a)} B={len(_comp_rows_b)}")
        print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via overlap parallel (O40).")
    else:
        comp_vals = _build_values_fast(companies_to_insert)
        comp_sql = _COMP_PREFIX + comp_vals + _COMP_SUFFIX
        leads_vals = _build_values_fast(leads_batch)
        leads_sql = (
            b"INSERT INTO leads (dot_number, status, agent_notes, lead_type) VALUES " + leads_vals + b" ON CONFLICT (dot_number) DO UPDATE SET status = 'New', lead_type = EXCLUDED.lead_type, updated_at = NOW()"
        )
...
print(f"GEN15_ELAPSED_SEC {_t_total_elapsed:.3f}")
print(f"GEN15_RPM {rpm:.0f}")
