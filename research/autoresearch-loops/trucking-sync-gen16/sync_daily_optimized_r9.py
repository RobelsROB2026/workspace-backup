...
if companies_to_insert and leads_batch:
    ...
    if conn2:
        # Build new company VALUES + leads VALUES in parallel (insurer uses unnest, no build needed)
        def _build_new():
            if _new_comp_rows:
                return _build_values_fast(_new_comp_rows)
            return None
        def _build_leads():
            return _build_values_fast(leads_batch)
        with ThreadPoolExecutor(max_workers=2) as _bex:
            _fn = _bex.submit(_build_new)
        ...
        # O49: Use unnest for insurer UPDATE — sends 2 arrays instead of 3933 VALUES tuples
