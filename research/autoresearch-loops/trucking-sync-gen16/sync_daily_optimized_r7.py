...
if companies_to_insert and leads_batch:
    ...
    if conn2:
        ...
        # O47: Fire new company INSERT + insurer UPDATE + leads in parallel
        def _exec_new():
            if new_vals:
                if _existing_dots is not None:
                    # Pure INSERT for genuinely new companies (no conflict possible)
                    cursor.execute(_COMP_PREFIX + new_vals)
                else:
                    cursor.execute(_COMP_PREFIX + new_vals + b" ON CONFLICT (dot_number) DO NOTHING")
        def _exec_insurer_and_leads():
            cur2 = conn2.cursor()
            if insurer_sql:
        ...
        with ThreadPoolExecutor(max_workers=2) as _dex:
            _f1 = _dex.submit(_exec_new)
            _f2 = _dex.submit(_exec_insurer_and_leads)
            _f1.result()
            _f2.result()
