...
if companies_to_insert and leads_batch:
    # O30: Build SQL values using adapt() instead of mogrify
    _t_build = time.perf_counter()
    with ThreadPoolExecutor(max_workers=2) as _mex:
        _f_fetched = _mex.submit(_build_values_fast, fetched_company_rows)
        _f_leads = _mex.submit(_build_values_fast, leads_batch)
    ...
    print(f"[PERF] SQL build (O30 adapt): {_t_build_elapsed*1000:.0f}ms")
