...
if companies_to_insert and leads_batch:
    ...
    if conn2:
        # O29: Sequential autocommit INSERTs on separate connections
        # Companies must complete first (leads has FK to companies)
        cursor.execute(comp_sql)
        cur2 = conn2.cursor()
        cur2.execute(leads_sql)
        cur2.close()
        print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via sequential autocommit (O29).")
    else:
