...
if companies_to_insert and leads_batch:
    ...
    # O34: Split company INSERT into 2 halves on 2 connections (parallel), then leads
    if conn2:
        # Build leads SQL while we set up the company split
        ...
        # Split company vals at a comma boundary near the midpoint
        comp_vals_bytes = comp_vals
        mid = len(comp_vals_bytes) // 2
        # Find the next "),(" boundary after midpoint to split cleanly
        split_pos = comp_vals_bytes.find(b"),(", mid)
        if split_pos > 0:
            comp_half1 = comp_vals_bytes[:split_pos + 1]  # includes the closing )
            comp_half2 = comp_vals_bytes[split_pos + 2:]  # starts after the ,
            comp_sql1 = _COMP_PREFIX + comp_half1 + _COMP_SUFFIX
            comp_sql2 = _COMP_PREFIX + comp_half2 + _COMP_SUFFIX
            # Fire both halves in parallel
            def _exec_half1():
            ...
            with ThreadPoolExecutor(max_workers=2) as _dex:
                ...
                _f1.result()
                _f2.result()
            # Now safe to insert leads (all companies committed)
            cursor.execute(leads_sql)
        else:
            # Too small to split — single INSERT
            comp_sql = _COMP_PREFIX + comp_vals + _COMP_SUFFIX
            cursor.execute(comp_sql)
            cursor.execute(leads_sql)
        _t_build_elapsed = time.perf_counter() - _t_build
        print(f"[PERF] SQL build+exec (O34 split): {_t_build_elapsed*1000:.0f}ms")
        print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via split parallel autocommit (O34).")
    else:
        comp_sql = _COMP_PREFIX + comp_vals + _COMP_SUFFIX
        leads_vals = _build_values_fast(leads_batch)
        leads_sql = (
