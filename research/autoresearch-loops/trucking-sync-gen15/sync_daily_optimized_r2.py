...
if companies_to_insert and leads_batch:
    ...
    # O35: Build 2 company VALUES blobs + leads VALUES in parallel (3 threads)
    if conn2 and _comp_rows_b:
        _build_exec = ThreadPoolExecutor(max_workers=3)
        _f_vals_a = _build_exec.submit(_build_values_fast, _comp_rows_a)
        _f_vals_b = _build_exec.submit(_build_values_fast, _comp_rows_b)
        _f_leads_vals = _build_exec.submit(_build_values_fast, leads_batch)

        comp_vals_a = _f_vals_a.result()
        comp_vals_b = _f_vals_b.result()
        leads_vals = _f_leads_vals.result()
        ...
        # Fire both company halves in parallel
        def _exec_half1():
        ...
        print(f"[PERF] SQL build+exec (O35 pre-split): {_t_build_elapsed*1000:.0f}ms | A={len(_comp_rows_a)} B={len(_comp_rows_b)}")
        print(f"UPSERTED {len(companies_to_insert)} companies + {len(leads_batch)} leads via pre-split parallel (O35).")
    else:
