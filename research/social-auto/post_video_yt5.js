...
(async () => {
  ...
  try {
    console.log("Wait for the title box...");
    const titleBox = await page.waitForSelector('div[aria-label="Add a title that describes your video (type @ to mention a channel)"]', { timeout: 60000 });
    ...
    console.log("Wait for the desc box...");
    const descBox = await page.waitForSelector('div[aria-label="Tell viewers about your video (type @ to mention a channel)"]', { timeout: 10000 });
    await descBox.click();
    await randomDelay(1000, 2000);
    await page.keyboard.type(caption, { delay: 45 });
    
    // "Not made for kids" radio button
    const notKidsRadio = await page.locator('tp-yt-paper-radio-button[name="VIDEO_MADE_FOR_KIDS_NOT_MFK"]');
    await notKidsRadio.click();
    await randomDelay(1500, 3000);
    ...
    const publicRadio = await page.locator('tp-yt-paper-radio-button[name="PUBLIC"]');
